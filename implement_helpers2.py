...
helper_script = """
        ...
        function getAnticoagulationStatus(p) {
            ...
            if (hasOak && hasHeparin) warnings.push("⚠️ OAK + Heparin (Bridging?)");

            // Status Logic
            if (hasOak && aptCount == 1) statusText = "OAK + SAPT";
            else if (hasOak && aptCount > 1) statusText = "Triple-Therapie (OAK + DAPT)";
...
content = content.replace("function rebuildPatientMap() {", helper_script + "\n        function rebuildPatientMap() {")
